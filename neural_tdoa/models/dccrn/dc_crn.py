...
from .complexnn import (
    ...
    NaiveComplexLSTM, ComplexBatchNorm,
    complex_cat
)
...
class DCCRN(nn.Module):
    ...
    def __init__(
        ...
        self._add_rnns(self, rnn_layers, bidirectional, fac)

        for idx in range(len(self.kernel_num) -1, 0, -1):
            self.decoder.append(
                _decoder_block(idx, self.kernel_num, self.kernel_size, use_cbn)
            )
    ...
    def _add_rnns(self, rnn_layers, bidirectional, fac):
...
def _decoder_block(idx, kernel_num, kernel_size, use_cbn):
    if idx == 1:
